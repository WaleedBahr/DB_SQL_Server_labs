...
BEGIN
   ...
   (SELECTconvert(nvarchar(50) ,ST_ID )FROM inserted) +' in table Student')
END;
...
' Insert New Row with Key=',+convert(nvarchar(50), @keyInserted), ' in table Student'))
INSERT INTO Student(St_Id) VALUES (400)
8-
create trigger tgStudAuditDel
on Student
instead of delete
as
...
insert into StudentAudit
values (suser_name(),getdate(),'Try to Delete Row with Key= '+ convert(nvarchar(10),@KeyValue))
or
  INSERT INTO ST_AUDIT
   VALUES(SUSER_NAME(),GETDATE(), SUSER_NAME()+'Try to Delete Row with Key = '+ 
   (SELECTconvert(nvarchar(50) ,ST_ID )FROM inserted) +' in table Student')
